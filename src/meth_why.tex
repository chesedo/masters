...
\label{sec:meth_why}
To answer the question of why methodologies were created, we conducted a systematic literature review \todo{Define systematic lit review}.
This review consisted of the following identical search across six publications\todo{define collections}.

\todo{paragraph of the 3 aspects}
\todo{back to passive voice}
...
The results will be discussed in chronological order, grouped in 10-year periods. \todo{the first methodology was documented in 1956 but this search first result is from the 1970s}

\subsection{1970s}
In this period software development is still new with many practices being borrowed from engineering.
So\todo{remove these} we find that the structured methodologies of the time promise to improve programmer's productivity.
...
However, engineers still wrote most software to perform a specific task, for example flying Project Apollo\todo{what are the physical aspects of project apollo}.
...
At the same time it would not be possible to update the program flying Apollo in mid-flight. \todo{qualify}
...
The term methodology was retained to describe this combination of phases and tools \cite{beregi_1985}. \todo{See if there is a thausuris tool}
...
Managing time became important as it had an impact on cost and timeliness ensures the system is not obsolete by the time it is operational because it took too long to be developed \cite{peacham_1985, beregi_1985, mannino_1987, paul_1993}. \todo{search for uses of `also`}
This completes the triad of cost, time, and quality being important, but for any project, only two could be chosen.\todo{where has quality gone}\todo{can only take away if there is something else about quality}
...
\subsection{1990s}
By the start of the early 1990s, methodologies which integrate the entire development process were trending.\todo{check be verbs}
...
Overall the reasons for using methodologies in the 1990s were to:\todo{time, cost and quality}
\begin{itemize}
...
There were still 3 articles though that listed the following reasons for using a methodology: \todo{same grouping as earlier} \todo{a diagram/visual.... in summary}
\begin{itemize}
